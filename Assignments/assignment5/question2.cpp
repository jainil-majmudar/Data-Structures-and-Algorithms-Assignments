...
class LinkedBinaryTree {
 ...
 public:
  ...
  void deleteSubtreeExcess(Node* node);
...
class LexLessThan {
  public:
  bool operator()(LinkedBinaryTree& a, LinkedBinaryTree& b) { 
...
void LinkedBinaryTree::deleteSubtreeMutator(mt19937& rng) {
  ...
  PositionList pl = positions(); // list of all positions
  int rand = randInt(rng, 0, size() - 1); // generates random integer
  int i = 0;
  Position target;
  for (auto iteration = pl.begin(); iteration != pl.end(); iteration++) {
    if (i == rand) {
      target = *iteration;
      break;
    }
  ...
  Node* leftChild = target.left().v; // gets node from its position
  Node* rightChild = target.right().v;
  deleteSubtreeExcess(leftChild);
  deleteSubtreeExcess(rightChild);
  target.setLeft(nullptr);
  target.setRight(nullptr);
  target.v->elt = randChoice(rng) ? "a" : "b";
}

void LinkedBinaryTree::deleteSubtreeExcess(Node* node) {
  if (node == NULL) {
    return;
  }
  deleteSubtreeExcess(node->left);
  deleteSubtreeExcess(node->right);
  delete node;
}
...
void LinkedBinaryTree::addSubtreeMutator(mt19937& rng, const int maxDepth) {
  ...
  if (target.isExternal()) {
      randomExpressionTree(target.v,maxDepth-target.v->depth(),rng); 
  }
}
...
int main() {
  ...
  const int NUM_TREE = 600;
  const int MAX_DEPTH_INITIAL = 1;
  const int MAX_DEPTH = 4;
  const int NUM_EPISODE = 6;
  const int MAX_GENERATIONS = 1400;
