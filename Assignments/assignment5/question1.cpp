...
class LinkedBinaryTree {
    ...
    public:
        ...
        Node* recreateTree(const Node* node); // recreates the tree
        LinkedBinaryTree& operator=(const LinkedBinaryTree& t);
        int size() const; // number of nodes
        bool empty() const; // is tree empty?
        Node* root() const; // gets the root
...
LinkedBinaryTree::LinkedBinaryTree(const LinkedBinaryTree& t) {
    score = t.getScore();
    n = t.size();
    _root = recreateTree(t.root());
}

LinkedBinaryTree::Node* LinkedBinaryTree::recreateTree(const Node* node) {
  if (node == NULL) return NULL;
  Node* new_node = new Node;
  new_node->elt = node->elt;

  new_node->left = recreateTree(node->left);
  new_node->right = recreateTree(node->right);

  if (new_node->left != NULL) new_node->left->par = new_node;
  if (new_node->right != NULL) new_node->right->par = new_node;
...
LinkedBinaryTree createExpressionTree(string postfix) {
    ...
    string token;
    LinkedBinaryTree::Node *temp;
    int count = 0;

    while (getline(ss, token, ' ')) { // tokenizes the string for every space 
        temp = new LinkedBinaryTree::Node(); // creates a new tree and initializes it to root
        temp->elt = token;
        temp->left = NULL;
        temp->right = NULL;
        count++;
        ...
        stack.push(temp); // pushes root of the tree to the stack 
    }
